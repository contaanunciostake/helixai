  ...
  const menuSections = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'afiliados', label: 'Afiliados', icon: DollarSign },
      ]
    },
    {
      title: 'Financeiro',
      items: [
        ...
        { id: 'comissoes', label: 'Comissões', icon: Package },
      ]
    },
    {
      title: 'Sistema',
            ...
            {isSidebarOpen && (
              <div className="flex items-center space-x-2">
                <div className="relative">
                  <div className="absolute inset-0 bg-gradient-to-br from-purple-500 to-indigo-600 rounded-xl blur-sm"></div>
                ...
                <div>
                  <p className="text-white font-bold text-sm">AIRA Admin</p>
                  <p className="text-xs text-white/60 font-medium">Painel de Controle</p>
                </div>
              </div>
            )}
            <button
              onClick={() => setIsSidebarOpen(!isSidebarOpen)}
              className="relative text-white/60 hover:text-white p-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-purple-500/30 transition-all duration-300 hover:scale-110"
            >
                  ...
                  return (
                    ...
                    >
                      <div className={`relative flex items-center ${isSidebarOpen ? 'space-x-4 px-4 py-3.5' : 'justify-center px-3 py-3.5'} transition-all duration-300 border-l-4 ${
                        isActive
                          ? 'bg-gradient-to-r from-purple-500 to-indigo-600 text-white shadow-xl shadow-purple-500/40 border-l-white'
                          : 'border-l-transparent hover:border-l-white hover:bg-gradient-to-r hover:from-purple-500/90 hover:to-indigo-600/90 text-white/70 hover:text-white hover:shadow-lg hover:shadow-purple-500/30'
                      }`}>
          ...
          <div className="border-t border-white/10 p-4">
            <div className={`relative overflow-hidden rounded-xl bg-white/5 backdrop-blur-sm border border-white/10 p-3.5 ${
              ...
              <div className={`relative flex items-center ${isSidebarOpen ? 'space-x-3' : 'justify-center'}`}>
                {/* Avatar */}
                <div className="relative">
                  <div className="absolute inset-0 bg-gradient-to-br from-purple-500 to-indigo-600 rounded-full blur-sm"></div>
                  <div className="relative h-11 w-11 rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-white font-bold text-sm shadow-lg shadow-purple-500/30 ring-2 ring-white/10">
                ...
                {isSidebarOpen && (
                  <div className="flex-1 min-w-0">
                    <p className="text-white text-sm font-semibold truncate">{user?.nome || 'Administrador'}</p>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0 w-full relative">
        ...
        <header className="relative h-20 flex items-center justify-between px-8 flex-shrink-0 z-10">
          ...
          <div className="relative z-10">
            <h1 className="text-xl font-bold bg-gradient-to-r from-purple-400 via-indigo-400 to-violet-400 bg-clip-text text-transparent">
              {menuSections.flatMap(s => s.items).find(item => item.id === currentPage)?.label || 'Dashboard'}
            </h1>
            <p className="text-sm text-white/60 font-medium mt-0.5">Painel administrativo do sistema</p>
          </div>

          <div className="relative z-10 flex items-center space-x-4">
            {/* Notificações */}
            <button className="relative p-2.5 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 hover:border-purple-500/30 transition-all">
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-green-500/10 border border-green-500/30">
              ...
              <span className="text-xs text-green-400 font-semibold">Sistema Online</span>
            </div>
          </div>
        </header>
