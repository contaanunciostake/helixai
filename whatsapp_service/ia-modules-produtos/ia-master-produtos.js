...
export class IAMasterProdutos {
  constructor(anthropicKey, db, empresaId = null, botConfig = {}) {
    ...
    // Mapear campos do banco para campos padronizados
    this._normalizarBotConfig();
    ...
    this.memoria = new MemoriaContextoProdutos(anthropicKey, db, this.botConfig);
    this.preditor = new PreditorFechamentoProdutos(anthropicKey, this.botConfig);
    this.gerador = new GeradorRespostasProdutos(anthropicKey, this.botConfig);
    this.geradorPerguntas = new GeradorPerguntasEspontaneasProdutos(anthropicKey, this.botConfig);
    this.analisadorCoerencia = new AnalisadorCoerenciaProdutos(anthropicKey, this.botConfig);
    this.classificadorProdutos = new ClassificadorProdutos(anthropicKey, this.botConfig);
    this.validadorCompletude = new ValidadorCompletudeProdutos(anthropicKey, this.botConfig);

    const nomeBot = this.botConfig?.nome_bot || 'AIra';
    const nomeEmpresa = this.botConfig?.nome_empresa || 'Distribuidora';
  ...
  /**
   * Normaliza campos do botConfig (mapeia nomes diferentes)
   */
  _normalizarBotConfig() {
    const cfg = this.botConfig;

    // Mapear nome_atendente -> nome_bot
    if (cfg.nome_atendente && !cfg.nome_bot) {
      cfg.nome_bot = cfg.nome_atendente;
    }

    // Mapear empresa_nome -> nome_empresa
    if (cfg.empresa_nome && !cfg.nome_empresa) {
      cfg.nome_empresa = cfg.empresa_nome;
    }

    // Mapear estilo_linguagem -> tom
    if (cfg.estilo_linguagem && !cfg.tom) {
      cfg.tom = cfg.estilo_linguagem;
    }
  }

  /**
   * Atualiza configurações do bot (do CRM) - alias para compatibilidade
   */
  atualizarConfig(botConfig) {
    this.atualizarBotConfig(botConfig);
  }
  ...
  atualizarBotConfig(botConfig) {
    ...
    this.memoria.atualizarConfig(this.botConfig);
    this.preditor.atualizarConfig(this.botConfig);
    this.gerador.atualizarConfig(this.botConfig);
    this.geradorPerguntas.atualizarConfig(this.botConfig);
    this.analisadorCoerencia.atualizarConfig(this.botConfig);
    this.classificadorProdutos.atualizarConfig(this.botConfig);
    this.validadorCompletude.atualizarConfig(this.botConfig);

    console.log(`[IA-MASTER-PRODUTOS] ✅ BotConfig atualizado: ${this.botConfig?.nome_bot || 'AIra'}`);
  }
  ...
  /**
   * Verifica se é pergunta sobre IA (método público para compatibilidade)
   */
  isPerguntaIA(mensagem) {
    return this._detectarPerguntaSobreIA(mensagem) !== null;
  }

  /**
   * Gera resposta anti-IA (método público para compatibilidade)
   */
  gerarRespostaAntiIA() {
    return this._respostaNegandoIA('pergunta_ia').resposta;
  }
  ...
  /**
   * Formata resposta (método público para compatibilidade)
   */
  formatarResposta(resposta) {
    return this.gerador.formatarResposta(resposta);
  }
  ...
  getStats() {
    return {
      ...
      nome_bot: this.botConfig?.nome_bot || 'AIra'
    };
  }
