...
def api_status():
    ...
    try:
        ...
        # Log para debug
        print(f'[WHATSAPP] Status consultado - Empresa: {empresa.id}, Conectado: {empresa.whatsapp_conectado}, Bot: {empresa.bot_ativo}')
        ...
        print(f'[WHATSAPP] Erro ao consultar status: {str(e)}')
        return jsonify({'success': False, 'error': str(e)}), 500
    finally:
        session.close()
...
def api_ativar_bot():
    ...
    try:
        ...
        # Verificar se WhatsApp está conectado (apenas quando ATIVANDO o bot)
        # Quando desativando, não precisa verificar conexão
        if ativar and not empresa.whatsapp_conectado:
            # Verificar se há um número WhatsApp cadastrado (pode estar conectado mas flag desatualizada)
            if not empresa.whatsapp_numero:
                ...
                # Há um número cadastrado, provavelmente está conectado mas flag desatualizada
                print(f'[WHATSAPP] Sincronizando flag whatsapp_conectado para empresa {empresa.id}')
                empresa.whatsapp_conectado = True

        empresa.bot_ativo = ativar
        session.commit()

        print(f'[WHATSAPP] Bot {"ativado" if ativar else "desativado"} para empresa {empresa.id}')
        print(f'[WHATSAPP] Status - Conectado: {empresa.whatsapp_conectado}, Bot Ativo: {empresa.bot_ativo}')
        ...
        session.rollback()
        print(f'[WHATSAPP] Erro ao ativar/desativar bot: {str(e)}')
        return jsonify({'success': False, 'error': str(e)}), 500
    finally:
        session.close()


@whatsapp_bp.route('/api/sincronizar-status', methods=['POST'])
@login_required
def api_sincronizar_status():
    """API para sincronizar status do WhatsApp com o bot engine"""
    import requests
    ...
    try:
        ...
        # Tentar verificar status real no serviço WhatsApp
        try:
            response = requests.get(
                f'{WHATSAPP_SERVICE_URL}/api/session/status',
            ...
            # Serviço não disponível, usar dados do banco
            pass

        # Fallback: retornar status atual do banco
        return jsonify({
            'success': True,
            'message': 'Status atual do banco (serviço WhatsApp não disponível)',
            'whatsapp_conectado': empresa.whatsapp_conectado,
        ...
        session.rollback()
        print(f'[WHATSAPP] Erro ao sincronizar status: {str(e)}')
        return jsonify({'success': False, 'error': str(e)}), 500
    finally:
        session.close()


@whatsapp_bp.route('/api/status-public', methods=['GET'])
def api_status_public():
    """API para verificar status sem autenticação (DEV ONLY)"""
    session = db_manager.get_session()
    ...
    try:
        ...
        print(f'[WHATSAPP] Status consultado (public) - Empresa: {empresa.id}, Conectado: {empresa.whatsapp_conectado}, Bot: {empresa.bot_ativo}')
...
@whatsapp_bp.route('/api/ativar-bot-public', methods=['POST'])
def api_ativar_bot_public():
    """API para ativar/desativar bot sem autenticação (DEV ONLY)"""
    session = db_manager.get_session()
