...
def get_config():
    ...
    if not phone:
        print(f"[BOT API] [X] Telefone nao fornecido")
    ...
    try:
        ...
        if not empresa:
            ...
            if not empresa:
                print(f"[BOT API] [X] Nenhuma empresa ativa encontrada no banco!")
                print(f"[BOT API] " + "-"*40 + "\n")
                return jsonify({'error': 'Empresa não encontrada'}), 404

            print(f"[BOT API] [OK] Empresa encontrada (fallback): {empresa.nome} (ID: {empresa.id})")

        else:
            print(f"[BOT API] [OK] Empresa encontrada: {empresa.nome} (ID: {empresa.id})")

        # Buscar configuração
        print(f"[BOT API]  Buscando configuração do bot...")
        config = session.query(ConfiguracaoBot).filter_by(empresa_id=empresa.id).first()

        if not config:
            print(f"[BOT API] [!] Configuração não encontrada, criando padrão...")
            ...
            session.add(config)
            session.commit()
            print(f"[BOT API] [OK] Configuração padrão criada")

        print(f"[BOT API] [OK] Configuração carregada:")
        print(f"[BOT API]    - Auto resposta: {config.auto_resposta_ativa}")
        print(f"[BOT API]    - Enviar áudio: {config.enviar_audio}")
        print(f"[BOT API]    - OpenAI configurado: {'Sim' if config.openai_api_key else 'Não'}")
        print(f"[BOT API]    - ElevenLabs configurado: {'Sim' if config.elevenlabs_api_key else 'Não'}")
        print(f"[BOT API] ----------------------------------------\n")
        ...
        print(f"[BOT API] [X][X][X] ERRO CRÍTICO [X][X][X]")
